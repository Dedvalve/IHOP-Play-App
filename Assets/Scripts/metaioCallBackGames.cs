...
public class metaioCallBackGames : MonoBehaviour 
{
		...
		virtual protected void onTrackingEvent(List<TrackingValues> trackingValues)
		{
			...
			{
				if((t.coordinateSystemID == 1)&&(t.state == TrackingState.Found))//Lemonade
				{
					viewModel.StartLemonadeGame();
					#if UNITY_IPHONE
					Mixpanel.SendEvent("Start Drink Game iOS");
					#endif
					#if UNITY_ANDROID
					Mixpanel.SendEvent("Start Drink Game Android");
					#endif
				}else if((t.coordinateSystemID == 2)&&(t.state == TrackingState.Found))//Simon
					  {
						  viewModel.StartSimonGame();
							#if UNITY_IPHONE
							Mixpanel.SendEvent("Start Food Game iOS");
							#endif
							#if UNITY_ANDROID
							Mixpanel.SendEvent("Start Food Game Android");
							#endif
					  }
			}
